...
export default {
    ...
    methods: {
        addResume(classification) {
            ...
            if (classification === 'education') {
                this.tempEducationObj.push({
                    _id: `temp-${this.count}`, // Ensure each item has a unique id
        ...
        cancelCreating(tempObjIndex, classification) {
            if (classification === 'education') {
                this.tempEducationObj = this.tempEducationObj.filter(item => item.tempIndex !== tempObjIndex);
            } else if (classification === 'experience') {
                this.tempExperienceObj = this.tempExperienceObj.filter(item => item.tempIndex !== tempObjIndex);
            }
        },
        async getResume() {
    ...
    mounted() {
        this.getResume();
        var naviBtns = $("#navi .nav .nav-link");

        for (var i = 0; i < naviBtns.length; i++) {
            naviBtns[i].addEventListener("click", function () {
                ...
                <div class="col-lg-9 col-md-12">
                    <div id="page-1" class="page one resume-section">
                        ...
                        <div v-for="item in educationObj" :key="item._id">
                            <ResumeWrap :openInEditMode="false" :classification="'education'" :id="item._id"
                                :position="item.position" :organization="item.organization" :period="item.period"
                                :description="item.description" @resumeUpdated="getResume">
                                <template #icon>
                        ...
                        <div v-for="item in tempEducationObj" :key="item._id">
                            <ResumeWrap :openInEditMode="true" :classification="'education'" :index="item.tempIndex"
                                :id="item._id" :position="item.position" :organization="item.organization"
                    ...
                    <div v-for="item in tempExperienceObj" :key="item._id">
                        <ResumeWrap :openInEditMode="true" :classification="'experience'" :index="item.tempIndex"
                            :id="item._id" :position="item.position" :organization="item.organization"
                            :period="item.period" :description="item.description" @resumeUpdated="getResume"
                            @cancel="cancelCreating">
                            <template #icon>
